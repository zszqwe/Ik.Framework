...
namespace IBatisNet.Common.Utilities
{
	...
	/// &lt;properties url=îfile:///c:/config/my.properties?/&gt;
	/// </summary>
	public class Resources
	{
        ...
        private static string ApplyDataMapperNamespacePrefix(string prefix,string elementName)
        {
            return prefix + ":" + elementName.Replace("/", "/" + prefix + ":");
        }

        private static bool findListInterface(Type type)
        {
            var interfaces = type.GetInterfaces();
            foreach (var item in interfaces)
            {
                if (item == typeof(System.Collections.IEnumerable))
                {
                    return true;
                }
            }
            return findListInterface(type.BaseType);
        }

        private static void AutoConfigSqlMap(XmlDocument xmlDocument, Type type)
        {
            XmlAttribute namespaceAttribute = xmlDocument.CreateAttribute("namespace");
            namespaceAttribute.Value = type.FullName;
            ...
            string MAPPING_NAMESPACE_PREFIX = "mapping";
            string MAPPING_XML_NAMESPACE = "http://ibatis.apache.org/mapping";
            nameSpace.AddNamespace(MAPPING_NAMESPACE_PREFIX, MAPPING_XML_NAMESPACE);
            var statements = xmlDocument.SelectSingleNode(ApplyDataMapperNamespacePrefix(MAPPING_NAMESPACE_PREFIX, "sqlMap/statements"), nameSpace);
            var list = statements.ChildNodes;//xmlDocument.SelectNodes("sqlMap/statements");
            foreach (MethodInfo method in methods)
            {
                XmlNode statement = FindStatement(method.Name, list);
                if (statement != null)
                {
                    Type resultType = method.ReturnType;
                    Type listResultType = null;
                    if (resultType.IsGenericType)
                    {
                        var typeDefinition = resultType.GetGenericTypeDefinition();
                        bool isListResult = findListInterface(typeDefinition);
                        var gengericArgs = resultType.GetGenericArguments();
                        if (isListResult)
                        {
                            if (gengericArgs.Length > 1)
                            {
                                throw new IBatisNetException("∑µªÿ÷µ»Áπ˚ «¡–±Ì¿‡–Õ£¨∑∫–Õ≤Œ ˝÷ªƒ‹ «“ª∏ˆ");
                            }
                            resultType = resultType.GetGenericArguments()[0];
                            listResultType = typeof(List<>).MakeGenericType(resultType);
                        }
                    }

                    XmlAttribute attribute = xmlDocument.CreateAttribute("resultClass");
                    attribute.Value = resultType.FullName;
                    statement.Attributes.Append(attribute);

                    if (listResultType != null)
                    {
                        XmlAttribute listClassAttribute = xmlDocument.CreateAttribute("listClass");
                        listClassAttribute.Value = listResultType.FullName;
                        statement.Attributes.Append(listClassAttribute);
                    }
                    ...
                    if (parameters.Length == 1)
                    {
                        ...
                        attribute = xmlDocument.CreateAttribute("parameterClass");
                        attribute.Value = parameter.ParameterType.FullName;
                        statement.Attributes.Append(attribute);
                        string countStatementName = method.Name + "_Count";
                        XmlNode countStatement = FindStatement(countStatementName, list);
                        if (countStatement != null)
                        {
                            attribute = xmlDocument.CreateAttribute("parameterClass");
                            attribute.Value = parameter.ParameterType.FullName;
                            countStatement.Attributes.Append(attribute);
                        }
                    }
                    else if (parameters.Length > 1)
                    {
                        attribute = xmlDocument.CreateAttribute("parameterClass");
                        attribute.Value = "System.Collections.IDictionary";
                        statement.Attributes.Append(attribute);
                        string countStatementName = method.Name + "_Count";
                        XmlNode countStatement = FindStatement(countStatementName, list);
                        if (countStatement != null)
        ...
        private static XmlNode FindStatement(string id, XmlNodeList statements)
        {
            foreach (XmlNode s in statements)
            {
                if (s is XmlComment)
                {
                    continue;
                }
                else if (s.Attributes["id"].Value == id)
                {
                    return s;
                }
            }
...
#region copyright
/*
*.NETÂü∫Á°ÄÂºÄÂèëÊ°ÜÊû∂
*Copyright (C) „ÄÇ„ÄÇ„ÄÇ
*Âú∞ÂùÄÔºögit@github.com:gangzaicd/Ik.Framework.git
*‰ΩúËÄÖÔºöÂà∞Â§ßÂèîÁ¢óÈáåÊù•ÔºàÂ§ßÂèîÔºâ
*QQÔºö397754531
*eMailÔºögangzaicd@163.com
*/
#endregion copyright
