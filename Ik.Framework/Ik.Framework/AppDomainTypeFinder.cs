...
namespace Ik.Framework
{
    ...
    public class AppDomainTypeFinder : ITypeFinder
    {
        ...
        public IEnumerable<Type> FindClassesOfAttribute<T>(bool onlyConcreteClasses = true)
        {
            return FindClassesOfAttribute(typeof(T), onlyConcreteClasses);
        }

        public IEnumerable<Type> FindClassesOfAttribute(Type assignTypeFrom, bool onlyConcreteClasses = true)
        {
            return FindClassesOfAttribute(assignTypeFrom, GetAssemblies(), onlyConcreteClasses);
        }

        public IEnumerable<Type> FindClassesOfAttribute<T>(IEnumerable<Assembly> assemblies, bool onlyConcreteClasses = true)
        {
            return FindClassesOfAttribute(typeof(T), assemblies, onlyConcreteClasses);
        }

        public IEnumerable<Type> FindClassesOfAttribute(Type assignTypeFrom, IEnumerable<Assembly> assemblies, bool onlyConcreteClasses = true)
        {
            var result = new List<Type>();
            try
            {
                foreach (var a in assemblies)
                {
                    ...
                    if (types != null)
                    {
                        foreach (var t in types)
                        {
                            var attr = t.GetCustomAttribute(assignTypeFrom, true);
                            if (attr == null)
                            {
                                continue;
                            }
