...
{
    public class ApiRequestToDataBase : BufferProcessorService
    {
        ...
        public override void BufferProcess(IList<BufferItem> items)
        {
            ...
            {
                var requestInfo = (ApiRequestProcessInfo)item;
                ...
                SetDbValue(row, "lng", requestInfo.Lng);
                SetDbValue(row, "lat", requestInfo.Lat);
                SetDbValue(row, "region_code", requestInfo.RegionCode);

                SetDbValue(row, "client_imei", requestInfo.Imei);
                SetDbValue(row, "client_net_type", requestInfo.ClientNetType);
                SetDbValue(row, "interface_version", requestInfo.InterfaceVersion);
                SetDbValue(row, "client_version", requestInfo.ClientVersion);
                SetDbValue(row, "client_type", requestInfo.ClientType);
                ...
                SetDbValue(row, "process_status", requestInfo.ProcessState);
                SetDbValue(row, "process_desc", CheckStringLen(requestInfo.ProcessDesc, 200));
